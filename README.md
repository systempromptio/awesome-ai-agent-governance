...
- [Keel](https://github.com/KeelDev-tech/keel) - Open-core job-application autopilot with honesty gates: refuses to invent qualifications, fails closed on ambiguity. Apache-2.0, self-hosted, free.
...
- [Provenza](https://github.com/kironovlaziz-del/provenza#readme) - Self-hosted AI governance platform providing policy enforcement, prompt masking (PII/NER), shadow AI monitoring, and MLOps lifecycle support. Apache-2.0.
...
- [EVE Authority Lab](https://evecore.com/authority-lab) - Scans a declared agent surface (an OpenAPI spec, MCP server config, IAM policy, or agent tool definitions) and reports every consequential action the agent can take, where its authority exceeds the job, and the control that closes each gap. Verdicts are deterministic and LLM-free, findings are split into what is observable in the artifact versus what a static surface cannot establish, and the result is a signed report a third party verifies offline. No signup. By EVE NeuroSystems.
...
- [EVE CoreGuard](https://evecore.com) - Deterministic, pre-execution governance for regulated AI decisions. Returns ALLOW/BLOCK/MODIFY before an action runs - a zero-LLM verdict identical for identical inputs and attributable to a named rule - and emits a cryptographically signed per-decision certificate that a third party verifies offline and replays deterministically. Versioned policy packs for ECOA/Reg B fair lending, SR 26-2 model risk, HIPAA, and the EU AI Act. SaaS, VPC, or on-prem. By EVE NeuroSystems.
...
- [Enforcer Governor (GVNR)](https://github.com/instruxi-io/enforcer-governor) - Local guard that decides allow, deny, or ask before each agent action, through a Claude Code PreToolUse hook, an OpenAI- or Anthropic-shaped proxy, or an MCP server. Dollar-per-minute spend limits per agent and across agents are on by default, with caps on new agents and errors per minute, loop detection, and command rules (pipe-to-shell refused; tree deletes, force-pushes, credential access and deploys escalated to a human). Every decision is written to a hash-chained receipt naming the operator. Source-available under FSL-1.1-ALv2.
...
- [OrcaPromptVault](https://github.com/Continuum-AI-Corp/OrcaPromptVault) - Dated archive of what coding agents are actually told and handed: 119 system prompts and 16 tool-schema files from 43 products, 44 of them recorded off the wire by a local proxy with the command that reproduces each one. Claude Code declares 35 tools in an interactive session and 29 under `claude -p`, on the same version - the tool surface a policy gate has to cover is not a constant, and this is where to check it per version and per entrypoint. AGPL-3.0.
...
- [writ](https://github.com/writ-agent/writ) - One `writ.yaml` policy (native DSL, Rego or Cedar) checked before every tool call from Claude Code, Codex, Gemini CLI, Cursor, Windsurf, MCP clients and agent SDKs; every decision recorded in a hash-chained, tamper-evident ledger with signed receipts.
...
- [antiburn](https://github.com/antiburn/antiburn) - Local desktop app that checks coding agent sessions for common causes of token burn: sessions too deep, premium models in subagents, high reasoning effort, broken caching, and unused MCP servers, skills, and tools. Shows usage-limit meters and per-session context, cost, and tool breakdowns. Supports Claude Code, Codex, Cursor, Copilot, Pi, and more. No account or server. MIT.
...
- [Governed Autonomy Doctrine](https://governedautonomy.org/) - Versioned reference architecture for runtime governance of autonomous agents, organised as five architectural planes: agent identity and lifecycle, execution and tool governance, policy and compliance, human oversight and audit, and multi-agent trust and delegation. [RFC 001](https://governedautonomy.org/rfc/v4-conformance-layer/) proposes twenty-nine testable, mechanism-agnostic conformance criteria across those planes and is open for comment through 31 October 2026. Publishes [mappings](https://governedautonomy.org/standards/) to the NIST AI RMF, the OWASP Top 10 for Agentic Applications, and the IETF SCIM agent governance draft.
