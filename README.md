...
- [EVE CoreGuard](https://eveaicore.com) - Deterministic, pre-execution governance for regulated AI decisions. Returns ALLOW/BLOCK/MODIFY before an action runs — a zero-LLM verdict identical for identical inputs and attributable to a named rule — and emits a cryptographically signed per-decision certificate that a third party verifies offline and replays deterministically. Versioned policy packs for ECOA/Reg B fair lending, SR 26-2 model risk, HIPAA, and the EU AI Act. SaaS, VPC, or on-prem. By EVE NeuroSystems.
