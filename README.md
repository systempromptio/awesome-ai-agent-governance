...
- [Etch](https://etch.systems) - Signed audit chain for AI agent decisions across MCP client tools. Every event carries a hybrid Ed25519 + FIPS 205 SLH-DSA-SHA2-128f envelope; rows Merkle-chain into epochs whose roots anchor to both Sigstore Rekor and Bitcoin OpenTimestamps for third-party time proof independent of the operator. Portable per-agent identity (mint / session / rotate / grant / delegate / revoke) with a v1.0 envelope specification, JSON Schema, and conformance vectors published on Zenodo ([DOI 10.5281/zenodo.22154537](https://doi.org/10.5281/zenodo.22154537)). Offline reference verifier ships as `world-model-mcp` on PyPI. Anonymous no-signup tier: one POST returns bearer + MCP endpoint with 500 signed events over 14 days. MIT verifier, BSL 1.1 hosted. Maps to EU AI Act Article 12, SR 11-7, ISO 42001, NIST AI RMF, SOC 2 CC7.
