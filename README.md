...
- [OrcaPromptVault](https://github.com/Continuum-AI-Corp/OrcaPromptVault) - Dated archive of what coding agents are actually told and handed: 110 system prompts and 16 tool-schema files from 35 products, 35 of them recorded off the wire by a local proxy with the command that reproduces each one. Claude Code declares 35 tools in an interactive session and 29 under `claude -p`, on the same version - the tool surface a policy gate has to cover is not a constant, and this is where to check it per version and per entrypoint. AGPL-3.0.
